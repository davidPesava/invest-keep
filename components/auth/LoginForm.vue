...
<v-card>
	...
	<v-card-text>
		...
		<v-form @submit.prevent>
			<v-text-field label="Email" v-model.trim="loginForm.email" type="text" placeholder="you@email.com" solo-inverted />
			<v-text-field v-model="loginForm.password" :append-icon="showPassword ? 'mdi-eye' : 'mdi-eye-off'" :type="showPassword ? 'text' : 'password'" label="Password" hint="At least 6 characters" counter solo-inverted @click:append="showPassword = !showPassword" />
			<v-btn @click="login" depressed large color="primary">Log In</v-btn>
		</v-form>
		<div v-if="!forgotPassword.isForgotPasswordFormOpen" @click="handleForgotPasswordForm" class="login-form__forgot-password-link">
			I forgot my password
		</div>
		<v-form v-if="forgotPassword.isForgotPasswordFormOpen" @submit.prevent class="mt-12">
			<h3 class="mb-3">Password reset</h3>
			<v-text-field label="Email" v-model.trim="forgotPassword.email" type="text" placeholder="you@email.com" solo-inverted />
...
export default {
	...
	methods: {
		...
		resetPassword() {
			...
			firebase.auth().sendPasswordResetEmail(this.forgotPassword.email).then(function () {}).catch(function (error) {});
		},
