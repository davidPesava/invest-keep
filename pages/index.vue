...
<div>
	<div class="default-layout__menu">
		<v-row>
			...
			<v-col cols="12" md="6" :class="isMobileMenuOpen ? 'd-xs-block' : 'd-none d-md-block'">
			...
			<v-col cols="12" md="3" :class="isMobileMenuOpen ? 'd-xs-block' : 'd-none d-md-block'">
				<div v-if="currentUser" class="default-layout__menu-buttons pr-md-8">
					<log-out-button class="ma-2" />
					<div>
						<v-btn class="ma-2" color="secondary" href="/dashboard">
							To app
						</v-btn>
					</div>
				</div>
				<div v-else class="default-layout__menu-buttons pr-8">
					<v-btn class="ma-2" color="secondary" @click="handleRegisterForm">
						Get started
					</v-btn>
					<v-btn class="ma-2" color="primary" @click="handleLoginForm">
						Login
					</v-btn>
				</div>
			</v-col>
		...
		<v-row>
			<v-col v-if="currentUser" cols="12">
				<v-card raised class="mt-3">
					...
					<v-card-actions>
						<v-btn class="ma-2" color="error" href="/dashboard">
							Take me there
						</v-btn>
					</v-card-actions>
				</v-card>
			...
			<v-col v-if="!currentUser && isLoginFormOpen" cols="12" md="6" offset-md="3">
				<login-form class="mb-5" @updateUser=getDataFromChild />
			</v-col>
			...
			<v-row>
				<v-col cols="12" md="5" class="default-layout__topCover">
					<div>
						...
						<v-btn v-if="!currentUser" color="secondary" @click="handleRegisterForm">
							Get started
						</v-btn>
					</div>
				</v-col>
				<v-col cols="12" md="7">
					<img src="/old/c.jpg" alt="InvestKeep Main" class="default-layout__cover">
				</v-col>
			</v-row>
		</div>
		<defaultPageContent />
