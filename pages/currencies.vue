...
<v-layout column align-start class="pl-6">
			...
			<v-dialog v-model="dialog" transition="dialog-bottom-transition">
				<v-card class="py-6 px-6">
					...
					<v-form class="d-flex flex-direction-row align-center">
						<v-select :items="currencyForm.fromItems" label="From" v-model="currencyForm.selectedFrom"></v-select>
						<div class="px-2">
							<v-icon>mdi-arrow-right</v-icon>
						</div>
						<v-select :items="currencyForm.toItems" label="To" v-model="currencyForm.selectedTo"></v-select>
	...
	<v-dialog v-model="isHistoryOpen" max-width="600">
		...
		async addErate() {
			if (this.currencyForm.selectedFrom && this.currencyForm.selectedTo) {
				if (this.currencyForm.selectedFrom === this.currencyForm.selectedTo) {
					...
					hel.push({
						from: this.currencyForm.selectedFrom,
						to: this.currencyForm.selectedTo,
						full: this.currencyForm.selectedFrom + this.currencyForm.selectedTo
					})
